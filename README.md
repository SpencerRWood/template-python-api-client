...
A minimal, typed Python REST API client template with `uv`, Ruff, mypy, pytest, pre-commit, GitHub Actions, and semantic-release wired together.

## Intended Use

Use this template for reusable clients for consuming external or internal REST APIs. The repository infrastructure is ready
...
```text
src/template_python_api_client/
  __init__.py
  py.typed
  auth.py
  client.py
...
Keep reusable Python code under `src/template_python_api_client/` and tests under `tests/`.
...
- project name: `template-python-api-client`
- package name: `template_python_api_client`
